...
sub process_a_cmd
{
    ...
    my @icmds=split(';',$incmd);

    if ( scalar @icmds > 1 ){
        for my $icmd (@icmds){
            $self->process_a_cmd($icmd);
        }
    }
...
sub run
{
    ...
    while(!$self->done) {
        $self->process_a_cmd($icmd);
        last if $icmd;  # only loop if we're prompting for commands
    }
...
It takes any number of arguments, concatenates them together and
