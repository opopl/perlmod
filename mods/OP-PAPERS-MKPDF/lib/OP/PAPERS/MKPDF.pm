...
sub run() {
    ...
    foreach my $id ($self->optids) {
      $runopts->{$id}=0;
      $runopts->{$id}=1 if $self->_opt_true($id);
    }
...
sub write_tex() {
    ...
    push( @include, "p.$fpkey.nc.ssec" );
    push( @include, "p.$fpkey.titpage" );
...
sub write_tex_nc_ssec() {
    ...
    # Redefine the \ssec command
    open( NC, ">p.$fpkey.nc.ssec.tex" );
    print NC "% Date: $date\n";
    my $ssec = '\nc{\ssec}[1]{%' . "\n" . ' \renewtf{#1}%' . "\n" . '}%';
    print NC "$ssec\n";
    close(NC);
...
sub write_tex_title_page_revtex() {
    ...
    my $fpkey   = $self->fpkey;
    my $pname   = $self->pname;
    my $titpage = "p.$fpkey.titpage.tex";
