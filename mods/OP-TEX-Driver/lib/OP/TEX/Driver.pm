...
use Exception::Class ( 'OP::TEX::Driver::Exception' );
...
sub throw {
    my ($self, @args) = @_;
    OP::TEX::Driver::Exception->throw( error => join('', @args) );
...
OP::TEX::Driver - Latex driver

=head1 VERSION

This document describes version 0.08 of C<OP::TEX::Driver>.

=head1 SYNOPSIS

    use OP::TEX::Driver;

    $drv = OP::TEX::Driver->new( source  => \$doc_text,
                               output  => $filename,
                               format  => 'pdf',
                               %other_params );
    $ok    = $drv->run;
...
The OP::TEX::Driver module encapsulates the details of invoking the
...
C<OP::TEX::Driver> depends on latex and friends being installed.
