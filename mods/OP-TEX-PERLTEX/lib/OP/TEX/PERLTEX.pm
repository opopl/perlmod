...
our $sandbox_eval;
our $latexpid;
our $pipestring = "\%\%\%\%\% Generated by $progname\n\\endinput\n";

sub init_vars() {
	my $self=shift;

	my $latexprog=$ENV{"PERLTEX"} // "latex";
	$self->latexprog($latexprog);
}
...
sub get_opt(){
	...
	my($latexprog,$runsafely,$pipestring);
	...
	for my $v(qw(latexprog runsafely pipestring)){
		eval '$self->' . $v . '($' . $v . ');' ;
		die $@ if $@;
	}
...
sub run(){
	...
	for ($firstcmd=0; $firstcmd<=$self->latexcmdline_count; $firstcmd++) {
	    my $option = $self->latexcmdline_index($firstcmd);
