...
sub init_vars() {
	...
	my $self->latexprog=$ENV{"PERLTEX"} // "latex";
	$self->latexprog($self->latexprog);

	my $self->progname = basename $0;
	...
	my $default_options={
		...
		progname    => basename $0,
		jobname     => 'texput',
		pipestring	=> "\%\%\%\%\% Generated by $self->progname\n\\endinput\n"
	};
...
sub get_opt(){
	...
	my($self->latexprog,$self->runsafely,$self->pipestring,$self->usepipe,$self->styfile);
	my @permittedops;

	GetOptions("help"       => sub {pod2usage(-verbose => 1)},
           "latex=s"    => \$self->latexprog,
           "safe!"      => \$self->runsafely,
           "pipe!"      => \$self->usepipe,
           "synctext=s" => \$self->pipestring,
           "makesty"    => sub {$self->styfile = "noperltex.sty"},
...
###LOOP_LATEXCMDLINE
	...
	foreach my $option ($self->latexcmdline) {
		...
		do { $icmd++; next; } if $option =~ /^-/;

	    unless($option =~ /^\\/) {
	        $self->jobname( basename $option, ".tex" );
	        $self->latexcmdline_set($firstcmd => "\\input $option");
	    }
	    last;
	}

	$self->latexcmdline_push("") if $self->latexcmdline_count==-1;
	...
	$self->latexcmdline_set(
		$firstcmd => 
		    ...
		    '\plmac@fromfile', $self->fromperl,
		    '\plmac@toflag', $self->toflag,
		    '\plmac@fromflag', $self->fromflag,
	...
	delete_files($self->toperl, $self->fromperl, $self->toflag, $self->fromflag, $self->doneflag, $self->pipe);
...
###PERL_ERROR_MESSAGE
	    ...
	    push @macroexpansions, $result if defined $self->styfile && $optag eq "USE";
	    ...
	    close FROMPERL;
	    delete_files($self->toflag, $self->toperl, $self->doneflag);
	    ...
	    awaitexists($self->toperl);
	    delete_files($self->fromflag);
...
sub awaitexists ($)
{
    while (!-e $_[0]) {
        ...
        if (waitpid($latexpid, &WNOHANG)==-1) {
            delete_files($self->toperl, $self->fromperl, $self->toflag,
                         $self->fromflag, $self->doneflag, $self->pipe);
            undef $latexpid;
            exit 0;
        }
    }
...
\\ProvidesPackage{noperltex}
    [2007/09/29 v1.4 Perl-free version of PerlTeX specific to $self->jobname.tex]
STYFILEHEADER1
        ;
