...
sub determine_libs {
    ...
    if (defined $self->opt('libsadd')) {
        unshift @libs => split(',',$self->opt('libsadd'));
    }
    if (defined $self->opt('libs')) {
         @libs = split(',', $self->opt('libs') );
    }
...
OP::VimTag - Generate perl tags for vim
...
in the context of L<OP::VimTag> or which have some side-effects, you can act as
